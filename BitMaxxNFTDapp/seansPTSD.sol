...
contract PowerSurge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PowerSurgeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PPP", "PPP") {
        marketingWallet = payable(0xcF5Df13566d3A76967565193F531B4a40a44054E);
        ...
        dividendTracker = new PowerSurgeDividendTracker(payable(this), router, 0xA29D0ee618f33d8eFE9A20557fd0EF63dD050859,
                                                      0xc27BbD4276F9eb2D6F2c4623612412d52D7Bb43D,
                                                      "PPPTRACKER", "PPPTRACKER");
    ...
    function updateFees(uint256 deadBuy, uint256 deadSell, uint256 marketingBuy, uint256 marketingSell,
        ...
        require(deadBuy <= 200 && deadSell <= 500, "dead fees cannot exceed 2%");
        require(totalSellFees <= 1900 && totalBuyFees <= 1900, "total fees cannot exceed 9% sell or buy");
    ...
    function isReinvest(address account) external view returns (bool) {
     ...
     // Available balance after subtracting staked tokens added for frontend display
    function getAvailableBalance(address account) public view returns (uint256) {
        uint256 totalBalance = balanceOf(account);
        uint256 stakedBalance = tokensStaked1Month[account] + tokensStaked3Months[account] + tokensStaked6Months[account];
            return totalBalance.sub(stakedBalance); // Available balance after subtracting staked tokens
    }
 ...
 function StakeFor1Month(uint256 amount) public {
    ...
    amount = amount * 10**18;
    require(balanceOf(msg.sender) >= tokensStaked1Month[msg.sender] + tokensStaked3Months[msg.sender] + tokensStaked6Months[msg.sender] + amount, "Insufficient Available Balance to stake");
...
function StakeFor6Months(uint256 amount) public {
    ...
    function setDividendTracker(address NewAddress) public onlyOwner {
        dividendTracker = PowerSurgeDividendTracker(payable(NewAddress));
    }
...
contract PowerSurgeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PowerSurge public PowerSurgeContract;
    ...
    constructor(address payable mainContract, address router, address token, address token2, string memory _name,
        ...
        PowerSurgeContract = PowerSurge(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main PowerSurge contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PowerSurgeContract);

            uint256 prevBalance = PowerSurgeContract.balanceOf(address(this));
            ...
            {
                uint256 received = PowerSurgeContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PowerSurgeContract.transfer(account, received);
                }
                else {
                    success = false;
                }
